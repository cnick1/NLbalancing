function f2 = runExample32_balancedReduction(degree, scenario)
%runExample32_balancedReduction Runs the 3D academic example to
%visualize the nonlinear balanced reduction.
%
%   Usage:  runExample32_balancedReduction(degree,scenario)
%
...
switch scenario
    case 'initial-condition'
        ...
        title('Model output'); 
        legend('FOM output','ROM output using bar(z)','ROM output h(x)')
        ...
        %% Simulate the system's response to a random noise input to visualize the manifold
    ...
    case 'white-noise'
        ...
        plot3(X(:,1),X(:,2),X(:,3),'r')
        xlabel('$x_1$'); ylabel('$x_2$'); zlabel('$x_3$')
        exportgraphics(gca, sprintf('plots/example32_d%i_noise.pdf',degree), 'ContentType', 'vector');
