function [v,w] = runExample7(degree,plotEnergy,plotBalancing,balancingDegree)
%EXAMPLE7 Runs the example from the paper
%
%   Usage:  [v,w] = runExample7(degree,plotEnergy,plotBalancing,balancingDegree)
%
%   where
...
%%
  ...
  [A,B,C,N,Q] = getSystem7();
 
  %  Compute the polynomial approximations to the future energy function
  [w] = approxFutureEnergyQB(A,N,B,Q,C,eta,degree,true);
  futureEnergy{degree} = [];
  for k=2:degree
...
%         eFuture(i,j) = x.'*WBar*x;
  ...
  [v] = approxPastEnergyQB(A,N,B,Q,C,eta,degree,true);
  pastEnergy{degree} = [];
  for k=2:degree
    pastEnergy{k} = v{k}.'/2;
  end
