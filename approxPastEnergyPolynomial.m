...
%% k=3 case
if ( d>2 )
    NaVb = cell(l+1,d-1); % Pre-compute N_a.'*V_b, etc for all the a,b we need
    NaVb{1,2} = B.'*V2;
    NaVb{2,2} = g{2}.'*V2; % Newly needed for QB work
    Im = speye(m);
    ...
    b = -LyapProduct(H2.',v{2},2) ...
        - 2*kron(speye(n^3),vec(Im).')*vec(kron(NaVb{2,2},NaVb{1,2})); % New QB term
